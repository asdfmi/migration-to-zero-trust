...
func NewStatusCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(conn.Config.Gateways) > 0 {
				fmt.Fprintf(out, "\nGateways:\n")
				for i, gw := range conn.Config.Gateways {
					fmt.Fprintf(out, "  [%d] %s\n", i+1, gw.GatewayEndpoint)
					fmt.Fprintf(out, "      Tunnel IP: %s\n", gw.TunnelIP)
					fmt.Fprintf(out, "      CIDRs:     %s\n", strings.Join(gw.AllowedCIDRs, ", "))
				}
			}

			// Collect all CIDRs for routing check
			var allCIDRs []string
			for _, gw := range conn.Config.Gateways {
				allCIDRs = append(allCIDRs, gw.AllowedCIDRs...)
			}
