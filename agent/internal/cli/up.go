...
func makeApplyFunc(cpURL, ifaceName string, privKey wgtypes.Key, connPath string) func(controlplane.ClientConfig) error {
	return func(cfg controlplane.ClientConfig) error {
		if len(cfg.Gateways) == 0 {
			return errors.New("no gateways in client config")
		}

		var gateways []wireguard.GatewayPeer
		for _, gw := range cfg.Gateways {
			if gw.GatewayPublicKey == "" || gw.GatewayEndpoint == "" {
				return errors.New("gateway information is missing")
			}
			gateways = append(gateways, wireguard.GatewayPeer{
				TunnelIP:         gw.TunnelIP,
				GatewayPublicKey: gw.GatewayPublicKey,
				GatewayEndpoint:  gw.GatewayEndpoint,
				AllowedCIDRs:     gw.AllowedCIDRs,
			})
		}
